...
func DecodePacket(packetBytes []byte) (*layers.BFD, error) {
	...
	if p.ErrorLayer() != nil {
		// 解包失败
		log.Error("Failed to decode packet  %v", p.ErrorLayer().Error())
	...
	if !ok {
		// 没有BFD协议层
		log.Error("No BFD layer type found in packet")
	...
	// 解码成功
	log.Info(PACKET_DEBUG_MSG, pbfd.Diagnostic, pbfd.State, pbfd.Poll, pbfd.Final,
...
func checkLayers(p gopacket.Packet, want []gopacket.LayerType) error {
	...
	if len(layers) < len(want) {
		log.Error(" Number of layers mismatch: got %d want %d", len(layers), len(want))
...
func EncodePacket(Version layers.BFDVersion,
	...
	if err != nil {
		log.Error("serial error: %s" + err.Error())
		return []byte{}
	}
