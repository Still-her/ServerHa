...
func NewClient(local, remote string, family int, CONTROL_PORT int) (*net.UDPConn, error) {
...
func (s *Server) Start() error {
	...
	if s.Family == syscall.AF_INET6 {
		...
		if err != nil {
			log.Error("ResolveUDPAddr err: " + err.Error())
			return err
		}
		s.listener, err = net.ListenUDP("udp6", udpAddr)
		if err != nil {
			log.Error("ListenUDP err:" + err.Error())
			return err
		}
	} else {
		// ipv4
	...
	log.Info("udp server run at:", udpAddr.String())
...
func (s *Server) Loop() {
	for {
		...
		if err != nil {
			log.Error("read from udp error:" + err.Error())
			continue
		}
